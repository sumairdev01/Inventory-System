<x-app-layout>
    <div class="p-6 max-w-2xl mx-auto">
        <div class="mb-8">
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Supplier</h1>
            <p class="text-slate-500 mt-2 font-medium">Update profile details for <span class="text-indigo-600">"{{ $supplier->name }}"</span></p>
        </div>

        <div class="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
            <form action="{{ route('suppliers.update', $supplier->id) }}" method="POST" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="address" rows="3"
                              class="w-full px-4 py-3 rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all font-medium">{{ old('address', $supplier->address) }}</textarea>
