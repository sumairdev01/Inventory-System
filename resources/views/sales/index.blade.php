    ...
    <div class="p-6 max-w-7xl mx-auto space-y-6">
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            ...
            <a href="{{ route('sales.create') }}"
                class="bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white px-6 py-2 rounded-lg font-bold shadow-lg shadow-emerald-100 transition-all duration-200 transform hover:-translate-y-0.5 active:scale-95">
                New Sale
            </a>
        </div>
        <div class="bg-white shadow rounded-lg border border-slate-200 overflow-x-auto">
                    ...
                    @forelse($sales as $sale)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-4 py-4 text-right flex justify-end space-x-2">
                                <a href="{{ route('sales.show', $sale->id) }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">View</a>
                                <a href="{{ route('returns.sales.create', $sale->id) }}" class="text-emerald-600 hover:text-emerald-800 text-sm font-medium">Return</a>
            ...
            @if($sales->hasPages())
                <div class="px-4 py-3 bg-gray-50 border-t border-gray-200">{{ $sales->withQueryString()->links() }}</div>
            @endif
        </div>
    </div>
</x-app-layout>